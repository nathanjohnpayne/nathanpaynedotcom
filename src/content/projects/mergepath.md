...
There is no backend, no build system, and no network calls. The draft YAML panel shows what the current knob configuration would look like as `.github/review-policy.yml`, but it does not write to disk—copy it yourself if you want to apply. The canonical spec is `specs/mergepath_playground.md`; if the spec and the page disagree, the spec wins.
