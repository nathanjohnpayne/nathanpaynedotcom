...
constraints:
  - value: "260 fields"
    label: "the device model a partner's questionnaire is normalized onto, from 100–150 answered questions"
  - value: "5 feeds"
...
decisions:
  - title: "Extraction proposes; a human signs"
    ...
    evidence: "The gate is real rather than advisory: the approve route refuses server-side while anything is unreviewed, and the model's confidence has exactly one consumer—the review screen. What a live intake queue would settle is the open question: does mandatory review actually catch extraction errors, or does it only add reviewer friction?"
    status: "pending"
  - title: "An older snapshot does not silently win"
    ...
    evidence: "The warning, the per-row override set and the stale-overwrite count all shipped in the commit path, so the system counts every time a human let older data win and hands the number back."
    status: "pending"
  - title: "Freshness is shown, never enforced"
    ...
    evidence: "The staleness function has zero callers in the server code, so display is verifiably its whole job: the restraint is implemented, not merely intended."
    status: "pending"
  - title: "Aliases resolve; they do not merge"
    ...
    evidence: "The chain shipped in the import path with each resolution typed direct or contextual, and an unmatched name creates a partner rather than failing the file. Whether the registry absorbs real upstream drift or becomes permanent admin toil is the open question."
    status: "pending"
related:
...
<DecisionLedger decisions={props.decisions.slice(0, 1)} />
...
<DecisionLedger decisions={props.decisions.slice(1, 3)} />
...
<DecisionLedger decisions={props.decisions.slice(3)} />
