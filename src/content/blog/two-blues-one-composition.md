...
Reconstructed 2026-08-26 from `src/styles/global.css` at `ed24c72`, `main` on that date, with Lightning CSS 1.32.0 (via Vite 8.0.16 inside Astro 7.2.4—Astro's default CSS minifier, not esbuild). The commit matters: the toolchain named here is the one `ed24c72` pins, while `8bebc31`, the June commit PR #504 merged, declares `astro: ^6.1.0` and carries no lockfile, so it could not have produced this build:
...
The `color-mix()` count is the source-versus-artifact gap this exercise kept finding, and it needs its commit attached or the numbers look like they contradict each other. At `ed24c72`, the rebuild above, the source carries seventy-four calls and the artifact seventy-two. At `8bebc31`, the commit PR #504 merged and the state this post is about, it is seventy-six and seventy-four. A two-call gap either way. This post originally said the minifier "precomputes two," which is mechanically impossible—every one of those calls takes a `var()` custom property as its first color argument, and no minifier can resolve a custom property statically. What actually happens is duplicate-rule merging: two of the `color-mix()` declarations each appear twice in source and once after minification. Both pairs reproduce; only the explanation was wrong. The attribution matters too, because the count did not come from the two center tickets: it runs 6 at the pre-work baseline, 18 after #499, 21 after #500, and 76 after #503. The third refactor did most of it.
