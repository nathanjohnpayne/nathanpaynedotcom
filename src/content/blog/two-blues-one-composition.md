...
The `color-mix()` count is the source-versus-artifact gap this exercise kept finding: seventy-six calls in source, and seventy-two in the artifact counted below. This post has now been wrong about that gap twice, so it is worth being precise about what reproduces. It originally said the minifier "precomputes two," which is mechanically impossible—all seventy-six take a `var()` custom property as their first color argument, and no minifier can resolve a custom property statically. The replacement explanation, duplicate-rule merging, is wrong too: running this repository's own `lightningcss` over that file with `minify: true` returns seventy-six, not seventy-four. The seventy-four was measured against the deployed `global.XofGYe7g.css` during the 2026-06-11 audit and cannot be re-derived, because hash-named files are not addressable after a redeploy. What is left is the honest form: seventy-six in source, seventy-two in the named artifact below, and no account of the difference that survives checking. The attribution matters too, because the count did not come from the two center tickets: it runs 6 at the pre-work baseline, 18 after #499, 21 after #500, and 76 after #503. The third refactor did most of it.
