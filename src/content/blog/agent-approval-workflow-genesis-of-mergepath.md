...
**Evidence after launch:** four months on, the layering still bites—and it had grown a layer. While this post was being fact-checked, a `gh pr create` was refused because its body wrote `**Authoring-Agent:**` in bold. The refusal did not come from the hook, which passed the body along. In the months since launch the two had traded responsibilities: the hook gained an early exit for the author-wrapper path, and the wrapper gained a body contract of its own. That contract is line-anchored, and `^Authoring-Agent:` does not match a line starting with `**`. So the layer added later is the layer that caught it, and a bolded header is precisely the input that tells a line-anchored match from a substring one—two components, two readings of the same rule, one easy to mistake for the other. I have had this backwards in both directions across two earlier revisions, which is its own evidence for the point. The authorized break-glass merge that same session needed both local variables from the table above.
