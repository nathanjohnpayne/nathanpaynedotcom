...
After the task list at 01:36 UTC on August 27, rounds 1 through 11 ran with no input from me, including the round-9 choice that put a deletion engine in a provenance feature. From the afternoon on, the session asked me how to proceed five times, each time with three options. Here are all five, with the option I chose first in each.
...
The reviewers did not change. The contract did, and someone was there to hold it. And here is where [#1112](https://github.com/nathanjohnpayne/mergepath/pull/1112)'s guarantees came from, by round.

```mermaid title="Lineage of the #1112 findings" description="The original issue asked to record a source revision. Round 4 turned that into proving the recorded revision characterizes the mirrored bytes, which required a clean-tree check. Round 9 asked the resume path to honour that proof, and the author's chosen mechanism was rsync with delete plus a residue-reconciliation engine. Rounds 12 through 18 then found ten data-loss defects in that engine: the target root deleted, symlink referents followed, the filesystem root, linked-worktree .git files and glob escapes. The recut kept the hash, the trailer, the origin check and a plain clean-tree gate, and dropped the proof and the engine."
graph TD
...
I do not need to get better at recognizing bad engineering loops. The controls show that is the wrong job: long, repetitive, technically frustrating loops can be healthy. What the healthy ones had was somebody periodically asking whether the thing containing the error should exist.
