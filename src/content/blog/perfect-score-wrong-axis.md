...
keyTakeaways:
  ...
  - "This batch recorded 122 dispositions with zero rejections and still shipped a P1, posted by an unbriefed review 94 seconds after the final merge. The knowledge to catch it existed inside the batch, spec-derived and externally corrected, on a sibling PR—and no review lane carried it across."
...
That finding became [issue #809](https://github.com/nathanjohnpayne/mergepath/issues/809), a post-merge hotfix, and the subject of this post. The interesting thing is not that a bug shipped—bugs ship. It is the review record: 134 top-level finding threads across the batch and its hotfix, 116 of them severity-badged, and of the 122 threads with a recorded disposition—each marked addressed, deferred, or rebutted—not one rejected as factually wrong. By the only metric the process records, the review was perfect. The metric was measuring the wrong axis.
...
Nine PRs went up as one batch—[#789](https://github.com/nathanjohnpayne/mergepath/pull/789) through [#797](https://github.com/nathanjohnpayne/mergepath/pull/797), opened within thirty-six seconds of each other, later joined by [#800](https://github.com/nathanjohnpayne/mergepath/pull/800)—authored in a Claude agent session and pushed under my author identity, so the API shows me as author of all eleven. Each review lane gets its questions from a different place—the design's premise: the Codex App auto-reviews each new head, its brief the diff itself; CodeRabbit reads the same diffs as the advisory second opinion; the automated Phase 4b external reviewer posts merge-gating reviews under an agent identity different from the authoring agent's; and, new for this batch, the authoring session ran adversarial verifier agents re-running each PR's "is this test actually testing anything" experiment before approval—a lane I label an author record: it runs inside the session and leaves no GitHub trace. What every lane shares: each one's scope is a single PR's diff.

This batch taught me—twice, painfully—that agent-written summaries do not survive contact with the underlying record. None of the numbers below come from a summary: I pulled the raw review objects from the GitHub API and counted, keeping every denominator explicit.
...
All six were caught inside the batch, by its own verification. Another was caught by review and closed by nobody: a CodeRabbit thread on [PR #791](https://github.com/nathanjohnpayne/mergepath/pull/791) headlined "This assertion can never fail."—a `$( ... )` capture strips the trailing newlines the assertion exists to pin—and it sits among the twelve unmarked threads. A vacuous-guard finding left undispositioned, in the batch that dispositioned everything. Then there is the one the session could not see: the CommonMark preprocessor above—the same species, a wrong model of an external reality encoded into a scanner.

## Why sixteen passes missed what the seventeenth posted

My diagnosis went into [the batch retrospective](https://github.com/nathanjohnpayne/mergepath/issues/813#issuecomment-5133940688) the same day—posted under my agent's reviewer identity, attributing the analysis to me, so the linked comment is the agent's:
...
Which leaves the question I cannot answer, and the reason [#813](https://github.com/nathanjohnpayne/mergepath/issues/813) is an epic and not a patch. The repo is about to put a budget on review, because unbounded review does not terminate on its own. But a budget needs a unit, and every unit on the table is a count—passes, rounds, findings, approvals—and this batch just showed that a count optimizes toward spending every pass inside a single frame: exactly the failure mode made economical. Passes are not fungible; three passes from independent framings are worth more than ten from one. I know how to count passes. I do not yet know how to count frames.
