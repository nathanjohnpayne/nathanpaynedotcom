...
keyTakeaways:
  ...
  - "Fail-open defects compound where fail-closed defects merely block. Issue #1186 reports a wave audit whose watermark had not advanced since 2026-07-28, because an over-budget diff classified as reviewer unavailable, chained forward, and made the next range larger. It was filed 38 days later. A blocked pipeline announces itself within the hour; this one succeeded on every run."
  ...
  - "Issue #878 already specifies the general fix, one classification contract consumed by both the waiter and the gate, and has been open since 2026-08-03 at priority:high. Its guidance reads: do not attempt this as a patch series. Between September 14 and 15 I merged twenty distinct changes in 32 hours, five of them against #878. The enumeration is the tell, and I am the one enumerating."
pullquotes:
  - text: "A reviewer that is wrong is a cost you can price. A reviewer that is absent and counted as present is a system that will eventually pass something nobody checked, and will report that as a clean record."
    label: "The asymmetry"
    accent: red
  - text: "The audit had not advanced its watermark since July 28. It was not stuck in a way anyone could see. It was succeeding, every run, at doing nothing, and the reason it was doing nothing got structurally worse each time."
    label: "Compounding"
    accent: yellow
  - text: "The issue that describes the general fix opens with an instruction not to fix it in pieces. I read it, agreed with it, and then shipped five pieces in thirty-two hours."
...
sidebar:
  - type: text
    content: |
      ...
      Additions, deletions and file counts are each pull request's diff as GitHub reports it against its merge base. Review rounds are review submissions on the pull request timeline. Merge timestamps are the timeline's merge event where one exists; #1263 and #1169 have none, and their timestamps are the dates of the squash commits bearing their numbers, which is a different field and is labeled as such in the text.
  ...
  - type: text
    content: |
      Provenance, and what is not established. Every pull request figure, issue timestamp, label and quoted sentence comes from the GitHub API, read on 2026-09-23. Quotations from issue and pull request bodies are verbatim; several of those bodies were written by agents working under my account, which the text says where it matters.
...
The issue was filed on September 4. The watermark had last moved on July 28. **Thirty-eight days**, during which the audit ran, reported a plausible-sounding transient condition, and audited nothing, while the reason it was auditing nothing got structurally worse on every pass.

A fail-closed defect blocks, and a blocked pipeline announces itself within the hour because somebody cannot merge. This one succeeded every time. **Fail-open defects compound. Fail-closed defects merely block.** Blocking is expensive, visible, and gets fixed on Tuesday. The other kind gets fixed when someone happens to read a watermark.

## The Burst

Here is the part I did not notice while it was happening.

Between `2026-09-14T03:20:29Z` and `2026-09-15T12:01:26Z`, **32 hours and 41 minutes**, twenty distinct changes landed on `main` across 21 commits. The subject lines, in order, read like a concordance of one idea:
...
fix(coderabbit): refuse stale risk-marker fallback clearance (#1282)
...
Sixteen of the twenty are the same idea restated against a different surface: a value that meant "I could not answer" being read as an answer. Two of them, #1271 and #1283, carry `878` in their branch names.
...
It specifies the fix in four acceptance criteria. The first asks for "a written contract enumerating each observable signal, its source, and what it is permitted to prove, separating *the review ran* from *the review found nothing*." That distinction is the entire post in eleven words. The second asks for "one implementation of that contract, consumed by `scripts/coderabbit-wait.sh` and `scripts/coderabbit-severity-gate.sh` alike. No predicate exists in two copies."
...
That sentence was written on August 3. On September 14 and 15 I merged a patch series. Five of its members are recorded on #878's own decision banner as shipped against it, and #878 remains open at `priority:high` for "the remaining shared classification contract."
...
**The capability counterpoint has the defect inside it.** [#1169](https://github.com/nathanjohnpayne/mergepath/pull/1169) is the biggest thing merged all month, +15,436 / −706 across 50 files, adding "the fail-closed authorization boundary for a singleton native GitHub merge queue." Real capability, not defense. Its issue [#1058](https://github.com/nathanjohnpayne/mergepath/issues/1058) is still open and `status:blocked`, because "this repo's merge safety is **head-SHA-pinned by construction**, and a merge queue evaluates required checks on a different SHA."

Buried in that issue is a field I cannot stop looking at. It records `required_status_checks.strict` as "**unknown**—`GET /branches/main/protection` returns `403 Resource not accessible by integration`." The system could not read its own gate configuration, and what it recorded was not a failure. It was `unknown`, written down and carried forward. The one pull request I offered as the counterexample contains the thesis.
...
**Eleven of the 164 open issues in the repository are about a rate limit, a quota, or a timeout being mistaken for a verdict.** The oldest is [#722](https://github.com/nathanjohnpayne/mergepath/issues/722), at 78 days, whose body describes a poll that "keeps waiting for the full `review_timeout_seconds` window (840s default) and then exits `4` (FALLBACK_REQUIRED)—indistinguishable from a genuinely slow/no-op review," and notes that the detection that would fix it "lives **only** in the retrospective audit script, never in the live gate/trigger path." The median is 26 days. The newest, [#1305](https://github.com/nathanjohnpayne/mergepath/issues/1305), was filed today, carries no labels yet, and is titled "Phase 4b barrier treats a cap-exhausted Codex arm as a self-clearing wait."

Truncation is the exception that proves the shape. There are zero open truncation issues, because there was exactly one, #1247, and a 68-line pull request closed it the same minute it was filed. One known instance, one small fix, done.

The rate-limit family has eleven open instances because a rate limit arrives in eleven grammars: a status string, a comment, a missing comment, an expired notice, an aged summary, a prose paragraph the vendor writes differently this month, a silently empty array. Two of the eleven, #961 and #962, were filed **six seconds apart**, both split out of one earlier issue. That is not two discoveries. That is one problem being decomposed into tickets, which is what enumeration looks like from the inside.
...
I have been treating it as the second, and I can show you the cost. A repository where the largest category of a month's merged work is retrofitted skepticism about values the system already trusted. A thirty-two-hour stretch that felt like progress and was a list. A design document specifying the general fix, open at high priority for fifty-one days, whose first instruction is the one I did not follow. An audit that reported reviewer unavailability for thirty-eight days while auditing nothing. The largest instance closed unmerged after fifty-one minutes. Eleven more waiting, each one a small pull request nobody has had a reason to write yet.
