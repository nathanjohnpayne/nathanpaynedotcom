...
keyTakeaways:
  ...
  - "Findings per review round is a burn-down chart, and this one never landed: 57 findings across 24 rounds that dipped and rebounded, round 20 producing five and round 24 still producing two. The answer was not more rounds. Deleting the capability nobody had asked for removed the proof obligation generating the findings and ended the loop in a single commit—eighteen rounds after the series had already shown the same shape."
  - "Automated review removes the friction that used to stop a loop. Two review bots drew 256 review submissions and 126 inline findings across seven pull requests, and none of that rework needed anyone's approval or appeared as a line item. When the next round is nearly free, the signal to stop has to come from the shape of the series, because no budget will raise its hand."
