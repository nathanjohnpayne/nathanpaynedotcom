...
keyTakeaways:
  - "The expensive part of a requirement is rarely the part it states. One sentence of style guidance bundled three capabilities—detect the pattern, decide what counts as prose, rewrite the file—and the one nobody had requested was 17% of the implementation and tests combined but nearly all of the work that would not converge. Unbundle before estimating: cheap to build and impossible to finish is a real combination."
  - "Findings per review round is a burn-down chart, and this one never landed: 57 findings across 24 rounds that dipped and rebounded, round 20 producing five and round 24 still producing two. The answer was not more rounds. Deleting the capability nobody had asked for removed the proof obligation generating the findings and ended the loop in a single commit—three weeks after one query would have shown the same shape."
  - "Automated review removes the friction that used to stop a loop. Two reviewers ran on every revision across seven pull requests—256 review submissions, 126 inline findings—and none of that rework needed anyone's approval or appeared as a line item. When the next round is nearly free, the signal to stop has to come from the shape of the series, because no budget will raise its hand."
...
Worth being exact about what "should not block shipping" means here, because report-only did not make detection consequence-free, and I got this wrong twice before a reviewer pinned it down.
