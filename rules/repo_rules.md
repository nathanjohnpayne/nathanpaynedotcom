...
- **`@astrojs/markdown-remark` is a required devDependency even though no source
  ...
  `astro build` fails in `coerceLegacyMarkdownPlugins` (#630). Astro declared it as
  an **exact-version optional peer** through 7.2.x and **loosened it to a caret range
  in 7.3.x** (`astro@7.3.1` peers `^7.3.0`), so the invariant is now that the pin must
  **satisfy** that peer rather than equal it. **Both stay pinned exact in
  `package.json`** regardless: the exactness is what stops a floating side from
  pulling a newer `astro` whose peer the pinned processor no longer satisfies, which
  is how #630 broke, and a caret on the pin cannot express it. A floating
...
- **`css-tree` is declared as a devDependency even though the toolchain already installs it, and that is deliberate.** `tests/lifecycle-marker.test.js` reads the emitted stylesheet with a real CSS parser rather than with regular expressions, after thirteen of PR [#964](https://github.com/nathanjohnpayne/nathanpaynedotcom/pull/964)'s sixteen findings turned out to be one finding arriving five rounds running: a regex approximating the CSS selector grammar does not handle shape *X*. Round 5 is where the hand-rolled version started producing a **false** failure—it read `@layer components` as conditional and would have rejected a behaviour-preserving cascade-layer refactor—which is the worse direction and the reason this is a dependency decision rather than one more patch. `css-tree` over `postcss` because one library parses selectors as well as rules, and the modifier-versus-surface distinction the check needs is a selector-AST question; `postcss` would have meant declaring `postcss-selector-parser` alongside it. The package is in `node_modules` regardless as a dependency of Astro's toolchain, so the check would run undeclared—and would break the first time that toolchain drops it, silently, since a check that stops finding rules stops finding defects. Test-only: nothing ships from it, and it is not in the pin block above because no lockstep constrains it. See #967.
