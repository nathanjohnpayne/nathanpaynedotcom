...
The guard runs inside `npm test`, so it reports as `build-and-test`—one of the two checks in `.github/required-head-checks` that Dependabot auto-merge waits on before merging. That placement is the point rather than an implementation detail: a `scripts/ci/check_*` would report under `repo-lint-local`, a check run neither `dependabot-auto-merge.yml` nor `scripts/required-head-checks.sh` observes, and grouped Dependabot PRs are the exact vector that breached this section twice. See #825.
