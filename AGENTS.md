...
7. If the PR meets the threshold, proceed to Phase 4 (see REVIEW_POLICY.md § Phase 4). Phase 4 has two legs:
   ...
   - **Phase 4b—External review fallback** when 4a is unavailable or 4a fell back. It has two legs and they are tried **in this order**. **Leg 1 (first, not an alternative): the automated orchestrator.** When `.github/review-policy.yml` has `phase_4b_automation.enabled: true` and `mode: local`, run `scripts/phase-4b-review.sh <PR#>`. **Run it from a trusted `main`-ref checkout, never from the checkout of the PR under review** (mergepath#628)—the PR is passed by number and its diff reviewed as data, so a PR that edits the phase-4b scripts, the adapters, or `gh-as-reviewer.sh` cannot shape its own verdict. The orchestrator picks an external reviewer from `available_reviewers` whose agent differs from the authoring agent, runs that reviewer's headless CLI over the diff, and posts `APPROVED` (exit `0`) or `CHANGES_REQUESTED` (exit `1`) under the reviewer identity via `scripts/gh-as-reviewer.sh`. On exit `0`, run `scripts/codex-review-check.sh <PR#>` and merge; on exit `1`, fix as nathanjohnpayne, push, and re-run (cap at `phase_4b_automation.max_review_rounds`; on exhausting that cap, stop and escalate to the human per § Disagreements below—do **not** fall through to leg 2, which exists for when leg 1 could not run, not for when it ran and disagreed). **Leg 2: the manual CLI handoff**—only when leg 1 declines, i.e. exit `4` (`fell_back_to_manual: true`: non-conformant verdict, adapter error or timeout, PR head drift), exit `3` (usage/infrastructure error), or exit `5` (automation disabled or `mode` != `local`). Then post the handoff message (see REVIEW_POLICY.md § Handoff Message Format) and wait for an external reviewer identity (e.g., nathanpayne-codex) to post an `APPROVED` review via a separate agent CLI session, addressing feedback via back-and-forth. Full step-by-step and the complete exit-code table: REVIEW_POLICY.md § Phase 4b.
