...
4. **Post-merge audit:** On a recurring cadence (weekly or per-sprint), run an automated audit of merged PRs to catch any that bypassed review policy. Flag these in a report surfaced to Nathan Payne.
5. **Violation handling:** PRs identified as policy violations in post-merge audit must be retroactively reviewed by a human within one business day. Repeat violations from a specific agent configuration should trigger a review of that agent's permissions and autonomy level. Escalation owner: Nathan Payne.
