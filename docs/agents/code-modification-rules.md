...
- **Lifecycle state renders as a mark plus the word, and the mapping lives in one module.** `src/lib/lifecycle-marker.ts` maps the `status` enum to a `.state-marker--*` modifier; the homepage Builds row, the `/projects/` card kicker, and the detail page's STATUS cell all import `stateMarkerClass()` from it. Do not re-declare the mapping in a page—`tests/lifecycle-marker.test.js` fails a second copy under `src/`. On a detail page the mark belongs to the STATUS cell and nowhere else: not beside the `h1`, not in the breadcrumb, not on the CTA. `.state-marker` is `em`-sized so it tracks its label; do not give a surface its own size override. See specs/project-pages.md § Lifecycle marker.
