...
### Credential Hygiene
- This repo should not contain API keys, service-account JSON, or ADC credentials. GA Measurement IDs are public identifiers; anything write-capable is not — with one explicit, documented exception (below).
- **Documented exception — PostHog project (client) API key (`phc_…`).** Hardcoded in `src/components/posthog.astro`. This is PostHog's public, browser-side ingest key: it is *write-only* (it can send events, never read or manage project data) and is necessarily served in the page JavaScript to every visitor, so it is public-by-design — the same security class as the GA Measurement ID. It is hardcoded (rather than read from a `PUBLIC_*` env var) so the value is present in every build path; this repo has no `.env.tpl`/`op inject` pipeline wired for `PUBLIC_*` vars. The PostHog *personal* API key (`phx_…`), which can read and manage the project, is a true secret and must never be committed. Residual risk: as with any client-side analytics key (including the GA Measurement ID), anyone can read it from the live site and send spurious events; mitigate with PostHog's Authorized-domains setting, bot filtering, or key rotation — not by hiding the key. Granting any new third-party token an exception here requires the same public-by-design + write-only justification and explicit human sign-off.
