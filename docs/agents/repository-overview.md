...
| `src/plugins/rehype-figure-captions.mjs` | Rehype plugin—wraps images in `<figure>` with a `<figcaption>` from alt text. Does not number them. |
| `src/plugins/rehype-figure-numbers.mjs` | Rehype plugin—numbers images and Mermaid diagrams in one document-order sequence (#998). Runs last of the figure plugins. |
