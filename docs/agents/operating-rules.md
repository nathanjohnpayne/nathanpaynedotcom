...
The `/projects/` and `/blog/` indexes share the `.blog-grid` composition and repeat one six-row geometry and accent sequence from `src/lib/index-grid.ts`. Desktop rows use only the established 50% and 72% vertical axes exposed by `--col-post`, `--col-mid`, and `--col-post-wide`: `50/22/28 → 72/28 → 50/50 → 50/22/28 → 72/28 → 50/50`. The accompanying accents are red + paper, blue, black, yellow + paper, light blue, and red. Both sequences restart at row 7, while stack mode collapses every row to one column and hides the decorative accents. The blog's `Latest` label belongs to the opening row's trailing accent position rather than to any color class, so editorial state cannot change the shared palette placement.
