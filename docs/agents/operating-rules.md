...
**The morph runs between pixel track lists, driven by JS (#1049).** Every focus template mixes `fr` tracks with fixed-length ones, and a track list only interpolates when each track keeps the same kind of size, so a plain CSS transition between templates flips the whole list at its midpoint (the panel jumps). `morphTracks()` in `src/pages/index.astro` therefore resolves both endpoints to pixels (reading the current computed tracks, so an interrupted morph continues from where it is), sets them inline, and lets the stylesheet's `--motion-plane` transition run between them. When the morph is over, `clearMorph()` hands the tracks back to the stylesheet with transitions off. "Over" is the grid's own `grid-template-*` `transitionend`, or a `transitioncancel` that leaves no grid transition running, when `getAnimations()` shows one running; otherwise (reduced motion, transitions off, nothing to animate) it is `--motion-plane + --motion-settle`, with twice that as a backstop. Anything that waits on the settled grid (the content reveal, the scroll cue, the keyboard focus hand-off) goes through `morphTracks`' `settled` callback, which runs exactly once even when a later morph or `clearMorph()` supersedes it. Never change `data-focus` outside `morphTracks()` on desktop, and never add a fixed timer that tears down inline tracks: a timer cancelled late-running morphs so their `transitionend` never fired. The stack has its own media-query template and gets no pixel morph.
