...
Two things follow for anyone extending it. **The suite carries its own defect fixtures**—§ *the collector rejects every shape it has been fooled by*—which rebuild each historical shape and assert the collector finds it, alongside the false positives that must keep passing (`@layer`, a surface sizing its own status element, a class whose escaped name merely looks like the primitive). The one rule that polices the status *element* rather than its mark is closed the same way the fill vocabulary is—a modifier may declare **nothing** there, rather than a list of the properties that happen to reach the mark—so `transform` and `color` are rejected without being named. Add to that table when a new shape turns up; a negative assertion over a stylesheet that is currently correct is not known to reject anything, which is exactly how five rounds of holes each passed every check. Nesting is resolved before classification—`&` is replaced by the parent selector's own nodes—because a nested rule splits one selector across two, and neither half carries what the other needs; and class, pseudo-element and property identifiers are all decoded before matching, because css-tree keeps their source spelling and `::be\66 ore` is a real `::before`. Three further equivalences are modelled for the same reason—`[class~="x"]` names a class, declarations inside a group rule nested in a style rule belong to the enclosing selector under that condition, and `:nth-child(n of S)` carries a subject—so extend by asking *what else selects this element*, not *what other syntax spells this selector*. Two asymmetries in that question are load-bearing: `[class~="x"]` names one token among many while `[class="x y"]` names the element's whole class list (so an exact value that omits `state-marker` cannot match a mark at all), and `:not()` flips a requirement rather than removing it, so polarity is tracked through nesting rather than the pseudo being skipped. **And the surface list is derived twice**, from the `stateMarkerClass()` call sites and from the classes on rendered marks in `dist/`, because each derivation is blind where the other sees—and the two are asserted to contain each other. That mutual containment is what closes the source scanner's blind spots rather than the scanner itself: it is not a JavaScript lexer and cannot become one, because these are `.astro` files where a regex-literal heuristic misfires on markup (`</div>` reads as a regex opener), so a call site the masker eats fails as a surface that renders but is named nowhere. A call site whose surface argument is not a string literal fails the suite rather than contributing nothing (#968): a surface that is never policed and a surface that does not exist must not arrive as the same empty result.
