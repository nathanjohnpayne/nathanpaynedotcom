...
**Enforced in CI (`.github/workflows/build-and-test.yml`, #632):** every pull request and every push to `main` runs `npm ci` (plain, no `--legacy-peer-deps`—see #631), `npm test` (`astro build && vitest run`), and `npm run lint`. The test suite inspects rendered Mermaid SVG for the blog label-contrast contract; lint runs ESLint and Vale prose checks. Any failure blocks the PR. `npm run test:e2e` (Playwright) joined this required job in #1004. Before that nothing ran it: nine spec files and 369 assertions were maintained while no automated path executed any of them, and #1003 shipped a breakpoint change that would have hung a spec with `npm test` green throughout. It runs after `npm test` so `dist/` already matches the checkout for `global-setup.ts`, and it is required rather than optional on the evidence in #1004—three consecutive local runs at `retries: 0`, 369/369, no flakes. On observed flakes in CI, demote it to its own non-required job rather than deleting it. Dependabot auto-merge (`.github/workflows/dependabot-auto-merge.yml`) waits on this job's `build-and-test` check succeeding on the PR's current HEAD, in addition to the existing structural `lint` check, before it will merge.
...
npm run test          # astro build && vitest run   (~19s)
npm run test:e2e      # playwright test             (~49s)
