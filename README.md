...
On desktop, hovering or focusing a panel triggers a CSS Grid transition that expands it and reveals its content. In stack mode—any viewport under 1024px wide or under 840px tall, so phones, tablets, and very short desktop windows—panels stack vertically with all content visible.
...
At `max-width: 1023px` or `max-height: 839px` (the composition's width and height floors, #992):
