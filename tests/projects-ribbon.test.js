 ...
 * `width < 1024 || height < 840`, the composition's floors (#992, #1042,
 * #1044). The desktop Mondrian is a square sized `min(95vw, 95vh, 1280px)`, so
 * a short window shrinks it as a narrow one does; below 840px tall
 * Community's content overflows its own panel, and the page renders the
 ...
 * asserted the production regression instead of catching it (#1042). Both are
 * desktop again, and tests/desktop-composition.test.js pins the real-window
 * cases directly.
 */
const VIEWPORTS = [
 ...
 * Community's content overflows its own panel below 824px (#1044).
 ...
 *
 * Recalibrated with #992 and again with #1044. The old ceiling of 0.8 sat just
 * over 0.757, the maximum measured at 1280x700, a viewport that no longer
 * renders the composition at all. The tightest desktop reading is now 0.601,
 * at the 798px square of the 840px-tall height floor, and 0.481 at the width
 * floor. 0.7 keeps the job the ceiling always had: catch the line growing
 * back toward the full measure, where the ten-item line it replaced ran
 * 0.986. It is not a re-derivation of today's number.
 */
