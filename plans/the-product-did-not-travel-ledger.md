...
| --- | --- | --- |
| “what made the cruise work was never in the code” / project page: “was never in the engine that generalized” | UNPROVABLE | Software, prompts, hosting and circumstance contributed to the observed experience. Previous-day marking, rankings and the feed are implemented features; the two events do not isolate causal contributions. Check Five Across PRs #383/#394 and the original cruise debrief's dinner and competition answers. |
| “It is not why the guests stopped” / first revision: incidents “shaped the weekend” | UNPROVABLE | Last guest mark 09:57 precedes the recorded 14:38 crash by 4h41m. That establishes ordering, not the time of a decision to quit or whether host failure prevented a return. Original audit: Bodega board mark timestamps and PostHog exception timestamps. |
...
| Old-title redirects | Removed from `firebase.json`, with their test and the docs note. The file has been at `the-product-did-not-travel.md` since PR #1057 and only the display title changed; the traffic check above found no event at either alias, and neither route ever existed. |
