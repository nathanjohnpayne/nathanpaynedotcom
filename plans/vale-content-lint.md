...
The gate discovers tracked and unignored `.md` and `.mdx` prose across the repository plus standalone `.yaml` and `.yml` content under `src/content`. It excludes deliberate violation fixtures and the exact canonical/kit paths propagated to this consumer by mergepath's sync manifest, with canonical-source headers as a defense-in-depth fallback; those mirrors must be fixed at their named source rather than rewritten in this consumer.
