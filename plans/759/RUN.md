...
- 2026-08-29—**Page five by merge order (#758, friends-and-family-billing) opened as [#858](https://github.com/nathanjohnpayne/nathanpaynedotcom/pull/858).** Four decisions, four constraints, no learnings. Delta audit §C23–§C50, twenty-eight rows, pinned to `friends-and-family-billing@d70aa8ac9fca414777985bb7dc74faa0462690e6` and this repo at `582b91d`. Prose 999 → 1,154 (facts pass, +15.5%) → 1,124 (brevity pass, −2.6%). **Taken out of table order at the operator's direction**, ahead of #755 and #756.
  ...
  - **A live security finding came out of verifying an exhibit, not auditing prose.** `firestore.rules:74` is `allow read: if true` on `publicShares`, with no expiry or revocation term, and Firestore's `read` grants `list`. The sibling `shareTokens` collection scopes `allow list` to the owner one block up, so the case was understood and not applied here; there is no test suite for the rules file at all. **This was deliberately kept off the page**—publishing an enumeration path against live household financial data is not a portfolio decision—and raised to the operator instead. Neither the audit nor the coordinator tested it against production.
