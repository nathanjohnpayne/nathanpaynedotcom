...
| 3 | Seven pages, one PR each | in progress—#752, #757, #754 and #753 merged; #758 open; two to follow (#755, #756) |
...
| 4 | #753 | `mergepath` | §E | decisions, constraints, learnings | [#848](https://github.com/nathanjohnpayne/nathanpaynedotcom/pull/848), [#852](https://github.com/nathanjohnpayne/nathanpaynedotcom/pull/852), [#853](https://github.com/nathanjohnpayne/nathanpaynedotcom/pull/853) | **merged** `c1cd157`, `6434575`, `2cecb0b`—5 decisions, 4 constraints, 1 learning; delta audit §E28–§E62 (35 rows); 5 review rounds, 26 findings, all taken, 26/26 accounted, zero unresolved threads. #848's break-glass merge was refused by that session's permission layer, not by any gate; the owner merged it, and #852/#853 were follow-up passes |
...
| 8 | #758 | `friends-and-family-billing` | §C | decisions, constraints | [#858](https://github.com/nathanjohnpayne/nathanpaynedotcom/pull/858) | **open**—4 decisions, 4 constraints, no learnings; delta audit §C23–§C50 (28 rows) pinned to `friends-and-family-billing@d70aa8a`. Taken out of table order at the operator's direction, ahead of #755 and #756 |
...
- 2026-08-28—**Page four by merge order and table entry 4 (#753, mergepath), opened as [#848](https://github.com/nathanjohnpayne/nathanpaynedotcom/pull/848).** Five decisions, four constraints, one learning. Delta audit §E28–§E62, thirty-five rows, pinned to `mergepath@3d961050e203e8b7a55bb551e89aa4da834356f6`. Prose 1,531 → 1,368 words (−10.6%), a restructure rather than an expansion. The figure moved every single round—1,190 at the build commit, then 1,228, 1,261, 1,311, 1,368—because every correction narrows a claim and narrowing costs words. **The restructure's −22.3% has given back more than half of itself to accuracy across four review rounds**, which is the cleanest measurement anyone on this epic has produced of why the facts pass and the brevity pass must not be merged. Recount immediately before merge; Codex caught this entry stale once already.
  ...
  - **Five review rounds, 26 findings, every one correct and every one taken.** Per round: 9, 4, 4, 5, 4. Nothing was rebutted. **Two findings were defects the review process itself introduced**—round 3 caught §E52 still asserting an attribution §E61 had just refuted (an appendix does not correct a file), and round 5 caught the round-4 cleanup guard whitelisting `.bootstrap-*` files, which is exactly what an interrupted run leaves behind. Expect corrections to seed the next round's findings; that is the normal shape, not a sign the loop is broken.
  - **Round 2 refuted the audit itself, which is the strongest single argument for spending the rounds.** The page said "no savings claim exists anywhere in the repository," resting on §E51's sweep. `scripts/phase-4b/accounting.sh:575-578` computes `human_minutes_saved_estimate` from hardcoded 30- and 180-minute constants. §E51 had swept five paths and `scripts/phase-4b/` was not among them. **An absolute is only as wide as its search**—write the search into the row so the next reader can see where it stops. Recorded as §E62.
  - **Word count moved on every single round: 1,190 → 1,228 → 1,261 → 1,311 → 1,368.** The restructure's original −22.3% gave back more than half of itself to accuracy. That is the cleanest measurement this epic has produced of why the facts pass and the brevity pass must not be merged, and it is worth quoting at the next author.
  - **Both P1s were about a destructive command in a ledger reproduction block**, not about the page. A published ledger's commands get run by strangers; the right posture is not a cleverer guard but no deletion at all.
...
- 2026-08-29—**Page five by merge order (#758, friends-and-family-billing) opened as [#858](https://github.com/nathanjohnpayne/nathanpaynedotcom/pull/858).** Four decisions, four constraints, no learnings. Delta audit §C23–§C50, twenty-eight rows, pinned to `friends-and-family-billing@d70aa8ac9fca414777985bb7dc74faa0462690e6` and this repo at `582b91d`. Prose 999 → 1,154 (facts pass, +15.5%) → 1,124 (brevity pass, −2.6%). **Taken out of table order at the operator's direction**, ahead of #755 and #756.
  - **The audit refuted the page's centrepiece, and this is the third consecutive run where the delta audit overturned something the coordinator believed settled.** §C40: the invoice email a household member receives has never gone through the canonical renderer. `functions/index.js:1280` uses canonical HTML only when the caller supplies an `html` field, and only the `[Test]` send does; the settlement board's real Email Invoice action sends `body` only. `#161` never touched that file—eight files in its diff, `EmailInvoiceDialog.jsx` absent, its `queueEmail` call byte-identical before, after and today. **Before `#161` the test email and the invoice rendered identically**; the fix closed the gap on the observed surface and opened a new one that has stood since 2026-04-04. Verified independently by the coordinator before it reached the drafting agent.
  - **The strongest framing came from the audit, not the brief.** §C46: issue `#159`'s definition of done had four clauses. The two that became automated checks held and still hold; the two that stayed prose—an editor reflecting final output, tests spanning all three surfaces—were not met at merge and are not met today. That is the page's thesis and it was nobody's idea in advance: an invariant that is prose is an invariant nobody reviews.
  - **Two ledger rows were corrected by their own successors.** §C38 found §C17's supporting evidence wrong—the settlement board has four filter chips and none of them is Overpaid, where §C17 said the chips carry all four. §C37 found `ADR-0005` is not the four-state record but a decoy of the family §C17 already flagged; the real reason is `ADR-0004`'s ban on the write-off. **Both headline verdicts stood while their citations did not**, which is the §M rule that a corrected value is not more trustworthy than the claim it replaced.
  - **An acceptance criterion was unmeetable as written, and saying so was the right answer.** AC 9 asks for screenshots demonstrating visual consistency across editor, preview, email, settlement and share-page. §C40–§C42 establish that consistency across those five surfaces is not a claim the evidence permits, and §C47 found both published "sent email" screenshots are `[Test]` messages the author sent himself. The page ships one exhibit instead—Edit and Preview captured from the product's own `VITE_E2E_MODE` harness with the `seedPage` fixture—showing the two surfaces that are *not* meant to match, plus a Mermaid diagram of the three render paths. **Executing the criterion is what proved it wrong**; the same lesson as #757's card-synopsis defect.
  - **A live security finding came out of verifying an exhibit, not auditing prose—and the first draft published it anyway.** The details are in ledger §C29 and are not restated here. The coordinator decided the exploit path did not belong on a public page that links to the live product, told the operator so—**and then shipped a draft that spelled it out.** Codex caught it as the round-1 P1, correctly. The page now says the rules do not bound the read and the application enforces it, which is the honest claim without the recipe. Neither the audit nor the coordinator tested it against production, and **the real remediation is in the product repository, not in this prose**—it was escalated to the operator rather than worked around here. **The lesson is not "redact harder": a decision to withhold something has to be applied to the artifact, not merely recorded in the log that describes the decision.**
  - **The description sentence was deliberately left alone.** §C49 found all four of its clauses hold, and §C50 found it duplicated verbatim across seven surfaces including the OG template, a test that hard-codes the homepage copy, and four tailored résumés in the docs vault. Changing accurate copy would have cascaded into all seven for no accuracy gain. **Not every audited surface is a surface to edit.**
  - **The blog carries the same parity claim and was left to its own issue** ([#857](https://github.com/nathanjohnpayne/nathanpaynedotcom/issues/857)), on the operator's instruction. Three surfaces in `six-prs-one-bug-agent-failure-modes.md` plus three ledger rows, one of them a Mermaid `description=` attribute. The post had already been narrowed once, to "template body," and stopped one surface short—the signature defect of this epic, committed by a correction.
  - **Three of the four §5 gates were already red at the branch point** (`582b91d`, clean tree): `npm test` 1 failed / 588 passed (#855), `npx eslint .` 2 errors (#856), `npx astro check` 2 errors in `tests/responsive/mermaid-accessibility.spec.ts` (untracked). Only `lint-prose` was green. Post-change counts are identical—no regression—but **"Build and Test" is red on `main`, so §C's "CI still has to be green" cannot be satisfied by this PR alone.** Record the baseline in the PR body rather than letting it read as the content change's fault.
  - The responsive suite was run against this worktree's own dev server via a scratch Playwright config, because the committed config hardcodes port 4321 with `reuseExistingServer` and would have silently measured another session's tree. 42 assertions pass at 375, 768 and 1440, including the new route's Mermaid label-height test. One homepage overflow failure did not reproduce on two re-runs.
