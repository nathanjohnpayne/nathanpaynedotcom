...
3. **Fix the four counts that do not reproduce.** "13 Phase 4b loops" → **26** (16 under the adapter signature). "Seven issues" in the backlog → **nine**, enumerated, with the `#774` "backlog" link dropped—there is no umbrella issue. "Two large CodeRabbit reviews on `#797` before the rate limits" → **three before merge, and no rate limit ever hit `#797`**. "Rate-limited out of most of the batch" → two rate-limit notices in eleven PRs, both at batch open, neither on `#797`.
