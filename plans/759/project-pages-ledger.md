...
**The cross-check holds.** PostHog records **820** `mark_square` events against Firestore's **845** squares—820 reproduced here—a 25-mark gap that offline queueing explains and that §B11's `persistentLocalCache` is the mechanism for. The two systems also agree independently on **10** players with a bingo (§B31).
...
**SPLIT—the 7 is SUPPORTED and re-derived; the 48 is a derived ceiling, half in-repo and half EXTERNALLY SOURCED.** `reshuffle_card` fires **7** times over the window, from **3** distinct persons. The 48 is not a recorded figure anywhere: it is the roster times the per-player cap. The cap is in the repository and is exactly three—`specs/reshuffle.md:6`, "Reshuffle a hard Day Card: pristine-only, 3 per Event, rules-bound counter," restated at `:12` and enforced monotonically per `:50`—and the reshuffle itself shipped mid-cruise (`833d57e`, 2026-07-17, §B12). The roster of 16 is Firestore's (§B31). So 48 is sound arithmetic over one verified factor and one external one, and should be presented as a ceiling rather than as a count of anything observed. The sharper unreported figure: only **three of sixteen players** ever used a reshuffle at all.
...
4. ~~**§B15**—"16 bingos across 124 squares" is wireframe fixture data for an invented player, committed four days before the cruise sailed.~~ **Superseded 2026-08-27.** §B15 is SUPPORTED, the line was committed on disembarkation morning, and the page keeps the figures. Its replacement on this list is **§B34 with §B26**: the "23:00 freeze" sits nine hours from the freeze this repository derives, and the embark card's eleven bingo-ing players contradict the ten holding a bingo event-wide. Both are #820 figures, both are cited in the paragraph the rewrite will reuse, and both need settling first.
