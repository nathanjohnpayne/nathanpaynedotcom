...
|---|---|---|---|
| `composition-ii-red-blue-yellow-1930.jpg` | `cf3345af4c5c7456d061f853ed5e7749eae22392df87c8b1012a339c81f07ccd` | 1183 × 1200 | **none—untagged** (APP0/JFIF, APP1/Exif without `ColorSpace`, APP13/Photoshop) |
| `composition-large-blue-plane-1921.jpg` | `97e0ef7349d4346002045fa5b96339b8beb47a033284328a27adf8ab2f1b13cb` | 996 × 1200 | **none—untagged** (APP0/JFIF only) |
...
6. **§D2** *(rewritten per §N.1, which overturned the verdict this instruction was based on)*: keep both 76 and 74; they reproduce at `8bebc31`, and the two lost calls are identifiable. Replace only the word *precomputes* with *merges two duplicate rule bodies*. Do not drop the pair, and do not repeat the per-page-bundle speculation: `dist/_astro/*.css` is a single bundle. And attribute the count honestly—55 of the 76 came from PR #503.
...
12. **The sidebar Mermaid (frontmatter L37–52) and the inline Mermaid are byte-identical.** *(Corrected: the inline block is at L113–124. L141–155 is the `median_plane` Python sampler, not a diagram—verify the byte-identical claim against L113–124 before cutting either.)* #744's revision cut exactly this duplication. Either is enough to satisfy the `fill:#DA2418` pin; cutting one is the cheapest compression available here.
...
## N. Adversarial verification pass—12 defects, corrected

An independent verifier re-derived every measurement here against the same shipped files and commits. **This section supersedes anything above that contradicts it.** Three corrections were applied inline; the rest are recorded below. Two of them would have shipped a *false* claim inside the very passage meant to cure #742's provenance complaint, which is the failure mode worth naming: an audit correcting a sourcing problem is the last place that can afford an unsourced assertion.

**N.1—the headline WRONG on the minifier count was itself wrong.** The post's 76 and 74 are both right and both reproduce: source at `8bebc31` has 76 `color-mix(` calls, minified has 74. The two lost calls are identifiable—`color-mix(in srgb, var(--blue-contrast) 55%, transparent)` and the 82% variant, each declared twice in source and once after minification. `main` at `ed24c72`, measured 2026-08-26 with Lightning CSS 1.32.0 (via Vite 8.0.16 inside Astro 7.2.4), shows the same 2-call gap: `src/styles/global.css` 74 → `dist/_astro/global.CwkyM5F4.css` 72. Pinned to a commit because the figure moves whenever the stylesheet does. **Only the word *precomputes* is wrong; the mechanism is duplicate-rule merging.** The fallback speculation that the audit grepped one hashed per-page bundle is also unfounded—`dist/_astro/*.css` is a single bundle. §I item 6 becomes "keep 76 and 74, replace *precomputes* with *merges two duplicate rule bodies*."

**N.2—neither JPEG carries an embedded ICC profile.** `sips -g all` reports "sRGB IEC61966-2.1" because macOS ColorSync substitutes its default for untagged RGB; that is the tool supplying a value, not reading one. A raw marker walk finds no APP2 `ICC_PROFILE` in either file. The correct statement is **"no embedded ICC profile; untagged, conventionally interpreted as sRGB,"** and the absence is itself a finding—#742 asks specifically for the embedded profile, and the honest answer is that there is none. Applied inline to §A's table.

**N.3—§E4 reproduced the undercount §E2 flags.** §E2 establishes four implementation PRs and faults the post for omitting #503; §E4 then says three, sourcing to `refs.json`, which has no #503 entry. #503's head ref is `codex/issue-501-contrast-token-cleanup`, so the claim holds for **four** PRs, all on `codex/*`.

**N.4—the "seven corrections" figure is inconsistent and undercounts.** §C3's prose says two plus five; its own bullets enumerate three plus three plus a revision note. The enumeration drops #497's addendum rows **A4** and **A5** and #498's amendment **A4** (favicon.svg hardcodes `#c11d19`). Loosest correct count is **ten**. The ledger also quotes #498's revision note but truncates immediately before the clause naming the omission—"Validation amendments **A1–A4** unaffected."

**N.5—§D3's blanket WRONG on "the museum-derived values" overreaches.** #498's token table uses that exact phrase for red and blue. It tags red `#DA2418` and yellow `#F0C800` as "starting point" and gives blue `#0A5C9E` with no qualifier at all, so nothing shows the blue was tuned. **Narrow the verdict to the yellow**, which is the only value chosen against the scan and the one §G4 needs.

**N.6—§D1's "the same three hold in source" is false for one.** `git show 8bebc31:src/styles/global.css | grep -o 'rgba(' | wc -l` returns **24**. Only the plane literals are zero; the "no `rgba()` at all" result is a property of the minified artifact and is near-tautological for a minifier that hex-folds `rgba()`.

**N.7—§J's published recipe does not reproduce two of its own rows.** The script specifies the neutral planes only as "near-neutral by `s_min` inversion"—no saturation ceiling, no value split—and the gray mask sweeps in the white ground (545,221 px, over half the image), so that median isolates nothing. At the script's own default `s_min=0.18` the 1930 red median is `#DE2923`, so the tabled `#DE2922` / Δ (0, 1, 0) should read `#DE2923` / Δ (0, 1, 1). This is the artifact §G1 tells the drafting pass to publish verbatim, so it has to run as printed.
...
**N.11—source-line imprecisions that fail a re-check.** The six `[data-accent=*]` scopes run **L1365–L1405** at `9d6139f`, not L1367–1406. PR #499 reworded the comment at **L2317**, not L2316—the line issue #497's own criterion gives. And §I item 11's pin inventory omits `tests/responsive/mermaid-accessibility.spec.ts:121` and `src/plugins/rehype-figure-captions.mjs:35,39`; both break on a slug or image rename.
