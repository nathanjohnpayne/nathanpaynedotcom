...
Filed as #835 after an outbound-link audit for #831 / PR #832: the paragraph at `:74` attributed the hook's body contract to the wrapper. **The link resolved and the code was real; the citation still did not support the claim it was attached to.** It was hung on the phrase "the wrapper's body contract," and the code at the other end implements neither a wrapper nor a body contract—it is the hook's command-text marker check. Relevant evidence for a weaker claim, attached to a stronger one: `blog-revision-process.md` § "Semantically wrong citations" exactly, and the reason a reader spot-checking the link comes away reassured. The prose was wrong twice over—about which component ran the check, and about it reading a body at all—and an earlier draft of this row called the citation "correct," which preserved the same error one level up.
