...
1. **Cost module.** `functions/src/llm/cost.ts` exposes a small helper:
   - `recordUsage({ stage, model, inputTokens, outputTokens, latencyMs, applicationId?, fixtureId? })` — computes `$` per call from a per-model rate table, writes a `LlmCall` record to Firestore (`llm_calls/{uuid}`) with `{ stage, model, input_tokens, output_tokens, cost_usd, latency_ms, created_at, owner_uid, application_id, fixture_id }`, and returns the computed cost. `application_id` correlates a production call to an `Application`; `fixture_id` correlates an eval-harness call to a fixture/eval run. Both are optional on the helper (a call may have neither, e.g. ad hoc tooling), but production and eval call sites should always pass the one that applies.
   - `priceFor(model, { inputTokens, outputTokens })` — pure function, no I/O, trivially testable.
2. **Wire into every LLM call site.** `functions/src/llm/anthropic.ts`, `functions/src/llm/openai.ts`, and `functions/src/llm/embeddings.ts` call `recordUsage()` after every completion/embedding response, threading `applicationId` (production request context) or `fixtureId` (eval-harness context) through from the caller. Any new call site added later that does not record usage should be caught by a CI lint that greps for `anthropic.messages.create|openai.chat.completions.create|openai.embeddings.create` without a neighboring `recordUsage(`.
