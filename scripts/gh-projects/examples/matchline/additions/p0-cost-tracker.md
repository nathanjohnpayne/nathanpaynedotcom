...
1. **Cost module.** `functions/src/llm/cost.ts` exposes a small helper:
   - `recordUsage({ stage, model, inputTokens, outputTokens, latencyMs })` — computes `$` per call from a per-model rate table, writes a `LlmCall` record to Firestore (`llm_calls/{uuid}`) with `{ stage, model, input_tokens, output_tokens, cost_usd, latency_ms, created_at, owner_uid }`, and returns the computed cost.
   - `priceFor(model, { inputTokens, outputTokens })` — pure function, no I/O, trivially testable.
2. **Wire into every LLM call site.** `functions/src/llm/anthropic.ts`, `functions/src/llm/openai.ts`, and `functions/src/llm/embeddings.ts` call `recordUsage()` after every completion/embedding response. Any new call site added later that does not record usage should be caught by a CI lint that greps for `anthropic.messages.create|openai.chat.completions.create|openai.embeddings.create` without a neighboring `recordUsage(`.
