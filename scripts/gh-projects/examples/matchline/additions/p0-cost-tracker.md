...
1. **Cost module.** `functions/src/llm/cost.ts` exposes a small helper:
   - `recordUsage({ stage, model, inputTokens, outputTokens, latencyMs, correlation })` — computes `$` per call from a per-model rate table, writes a `LlmCall` record to Firestore (`llm_calls/{uuid}`) with `{ stage, model, input_tokens, output_tokens, cost_usd, latency_ms, created_at, owner_uid, application_id, fixture_id }`, and returns the computed cost. `correlation` is a required discriminated union: `{ kind: 'application', applicationId: string } | { kind: 'fixture', fixtureId: string } | { kind: 'ad-hoc' }`. The production and eval variants therefore cannot compile without the ID their rollups require; tooling that intentionally has no correlation must choose `ad-hoc` explicitly rather than getting there by omission.
