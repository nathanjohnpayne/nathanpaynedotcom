...
11a. The authoring agent runs `scripts/codex-review-request.sh <PR#>` to trigger or await a Codex review. If the Codex App's "Automatic reviews" setting has already caused Codex to review the PR on open (typical latency ~2 minutes for small PRs), the script skips posting `@codex review` and goes straight to polling. Trigger-only deduplication accepts a fresh configured-author comment only when its complete body is that command, matched case-insensitively; prose or quoted containment alone does not suppress the exact POST, though it can still cause provider activity.
...
12a. `codex-review-request.sh` polls the PR until one of the following:
     ...
     - **Account-/connection-level block (#722).** Codex answered the trigger with a plain comment reporting account-level quota exhaustion (`blocked_reason: "usage_limit"` — "You have reached your Codex usage limits for code reviews…") or that the App is not connected (`blocked_reason: "not_connected"` — "To use Codex here, connect your GitHub account…"). Such a comment carries no findings, no `Reviewed commit:` anchor, and no reaction, so it is not a review, verdict, or clearance signal — but it is terminal: re-polling or re-triggering cannot produce a review until a human acts (upgrade / add credits / connect the App). `codex-review-request.sh` detects the usage-limit state only when a non-verdict bot comment begins with the provider's documented account-quota response; incidental quota text in a task or review summary is non-terminal. The shared `scripts/lib/codex-failure-markers.sh` retains a separate broader pattern for the retrospective latency audit, which does not feed live terminal decisions. On a live block the requester **short-circuits the wait immediately** instead of running out `review_timeout_seconds`, and exits `4` (`FALLBACK_REQUIRED`) with the reason surfaced in the output JSON's `blocked_reason` field and in its log line. Route to Phase 4b as for a timeout, but name the real cause in the handoff (a human account action, not "wait longer") rather than reporting a generic latency timeout.
...
```bash
# Verify the identity a commit here would use, and where it comes from.
