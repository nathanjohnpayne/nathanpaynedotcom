...
**One primitive, one size.** All five states render the same box—`calc(0.72em + 2px)` square, sized in `em` so it tracks whatever label it sits beside. The rule sets `box-sizing: border-box` and restates the border in that width, and both halves are load-bearing (#959): the `*` reset does not match pseudo-elements, so under the default content-box the cored `ARCHIVED` variant's own `padding: 0.1em` grew its box instead of eating into its fill, and it rendered about 22% larger than its four peers on every surface that shows it. Measured, not read off the CSS: 11.56px against 9.48px on `/projects/`, and 17 device px against 14 in the résumé PDF. `tests/lifecycle-marker.test.js` § declarations pins it, and the PDF classifier holds all four variants to one size window.
