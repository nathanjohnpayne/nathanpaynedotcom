...
Content held inside its own horizontal scroll container is the one exception, and it is an exception to the first sentence only, never to the second: a code block or a Mermaid diagram may hold content wider than itself, but the overflow belongs to that container and never reaches the page. For a diagram that container is `.mermaid-figure__graphic`, the element inside the `<figure>`—the figure itself also holds the visible caption, and a caption in a sideways-scrolling box would slide out from under the diagram it captions (#989). `tests/responsive/overflow.spec.ts` encodes this by skipping any element with a scrollable ancestor. Mermaid relies on it in the article column below the stacked breakpoint (#894), so a diagram's own rect can legitimately be several times its column's width there. The blog sidebar relied on it too until #986: that column is too narrow for a wide diagram to be readable scrolled, let alone scaled, so it holds no diagram wide enough to overflow and no sidebar figure scrolls at any width.
